...
export interface BuildAibomInput {
  ...
  /**
   * BAA/DPA coverage per external provider, from the routing policy's
   * destination catalog (`provider → covered frameworks`). Lets each hosted
   * model component carry the compliance metadata the AIBOM otherwise lacks —
   * the assessment's noted v4.0 gap. Absent/empty → the provider is stamped
   * `baa-covered: none`.
   */
  coveredByProvider?: Record<string, readonly string[]>;
  uuid?: () => string;
  now?: () => string;
}
...
export function buildAibom(input: BuildAibomInput): CycloneDxDocument {
  ...
  if (input.profile.routerEnabled) {
    components.push({
      ...
      version: input.embediqVersion,
      description:
        'Decision-only Express dispatch service generated by EmbedIQ. '
        + 'Routes inbound prompts to local Ollama models by default and '
        + 'forwards escalations to the LLM gateway — it holds no provider '
        + 'credentials; the gateway enforces the PHI/PII egress guardrail. '
  ...
  for (const apiId of input.profile.externalApis ?? []) {
    ...
    const covered = input.coveredByProvider?.[apiId] ?? [];
    const baaProperty = { name: 'embediq:baa-covered', value: covered.length > 0 ? [...covered].join(',') : 'none' };
    if (!provider) {
      ...
      components.push({
        ...
        properties: [{ name: 'embediq:hosted-provider-id', value: apiId }, baaProperty],
      });
      continue;
    }
    components.push({
