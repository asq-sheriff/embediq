...
| Env var | Default | Type | Purpose |
|---|---|---|---|
| `EMBEDIQ_OUTPUT_TARGETS` | `claude` | list | Comma/space-separated list of any of the 18 target tokens — `claude` / `agents-md` / `cursor` / `copilot` / `gemini` / `windsurf` plus the local-AI / RAG / router / governance tokens (`continue-dev`, `aider`, `zed-ai`, `ollama`, `rag-scaffold`, `local-router`, `litellm`, `cyclonedx-aibom`, `oscal-component`, `oscal-ssp-fragment`, `provenance`, `audit-bundle`) — or `all`. Case-insensitive. See [user-guide/05-multi-agent-targets.md](../user-guide/05-multi-agent-targets.md). |
