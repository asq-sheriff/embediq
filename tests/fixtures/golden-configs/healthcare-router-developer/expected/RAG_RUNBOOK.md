...
**Clinical decision support with hybrid local + hosted AI**. This runbook covers
...
- `rag-hipaa-compliance.md` — PHI handling, BAA, six-year audit retention

These rule files are path-scoped to `rag/**` and
`src/rag/**`, so Claude Code loads them automatically
whenever you edit retrieval code.
...
- [ ] **HIPAA**: six-year retention on the audit log
      (Security Rule §164.316(b)(2)).
- [ ] **HIPAA**: BAA in place with any non-local LLM or
      embedding provider you route to.
- [ ] **HIPAA**: de-identification (Safe Harbor / Expert
      Determination per 45 CFR 164.514) if retrieval
      results flow outside treatment contexts.
